...
from smart_crossover.network_methods.pdlp import pdlp_for_mcf
...
from smart_crossover.solver_caller.solving import solve_ot, solve_mcf
...
def main():
    ...
    # ot = ot_instances[0]
    # gur_output = solve_ot(ot, solver='GRB', presolve=0, method='network_simplex')
    # sinkhorn_output = sinkhorn(ot, reg=1)
    # # barrier_output = solve_ot(ot, method='barrier', solver='GRB', barrierTol=1e-2)
    # output = network_crossover(x=sinkhorn_output.x, ot=ot, method='cnet_ot', solver='GRB', solver_settings=SolverSettings(presolve=0))
    # print(gur_output)
