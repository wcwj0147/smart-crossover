function [realCost, realOF, time, count1, bas] = DimReduction_Gurobi(A_mat,b,c,l,u,X)
...
%         p1 = U'\c_B; p2 = L'\p1; p = P\p2;
        ...
        if (max(sol(m+1:m+n).*sign(sol(m+1:m+n)))<1e-10) && (min([r(bas_1==-1);-r(bas_1==-2)])>-1e-6)
            flag = false;
        end    
    ...
    realCost(testtime) = c_1'*sol*c_max;
    realOF{int16(testtime)} = sol;
    
    bas = reshape(bas_1(1:m),1,m);
end
