...
CplexCost = zeros(N,1);
CplexTime = zeros(N,1);
CplexTimeBlur = zeros(N,1);
CplexTimeBlurCross = zeros(N,1);
...
for k = 1:N
    ...
    [CplexCostBlurCross,CplexOFBlurCross,CplexTimeBlurCross(k)] = CplexMCFBlurCross(A_mat,b,c,l,u,eps,num2str(k)+".log",IterLim);
    ...
    [CplexCostBlur,CplexOFBlur,CplexTimeBlur(k)] = CplexMCFBlur(A_mat,b,c,l,u,eps,IterLim);
...
CplexCross = CplexTimeBlurCross - CplexTimeBlur;
CplexTimeAve = mean(CplexTime);
CplexCrossAve = mean(CplexTimeBlurCross-CplexTimeBlur)
MyTimeAve = mean(Mytime)

error = (MyCost-CplexCost).*sign(MyCost-CplexCost);
Error = max(error)
