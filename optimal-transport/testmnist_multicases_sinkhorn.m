...
load("C:\Smart Crossover\mnist-matlab-master\mnist.mat");
width = test.width;
height = test.height;
...
dimRCost_general_sinkhorn = zeros(N,1);
%dimRCost_warm = zeros(N,1);
%dimRTime_warm = zeros(N,1);
dimRTime_general_sinkhorn = zeros(N,1);
subcount1_s = zeros(N,1);
subcount2_s = zeros(N,1);
...
for k = 1:N
    ...
    M=rand(d1,d2)+1; % 和crossover比, M要大致相同, 和simplex比, M要很不同
    ...
    [dimRCost_general(k),dimROT_general,dimRTime_general(k),subcount1(k),subcount2(k)] = dimReduction_general(a,b,M,T); 

    [dimRCost_general_sinkhorn(k),dimROT_general_sinkhorn,dimRTime_general_sinkhorn(k),subcount1_s(k),subcount2_s(k)] = dimReduction_general(a,b,M,X); 

    Mytime_warm(k) = crosstime(k) + pushtime(k) + dimRTime_warm(k);
    Mytime_general(k) = dimRTime_general(k);
    Mytime_general_sinkhorn(k) = dimRTime_general_sinkhorn(k) + t_Sinkhorn(k);
