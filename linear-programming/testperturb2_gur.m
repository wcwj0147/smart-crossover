...
timeall_pt  = zeros(length(problem),1);
...
%     continue;
    ...
    %[GurobiCostBlurCross(k),GurobiOFBlurCross(k),GurobiTimeBlurCross(k),Gurobivbasis,Gurobicbasis] = GurobiLPBlurCross(A_mat,b,c,l,u,eps);
    
    [GurobiCostBlur(k),GurobiOFBlur(k),GurobiTimeBlur(k),~,GurobiPi(k),GurobiSlack(k)] = gurLPBlur_pi(A_mat,b,c,l,u,eps,sense);
          
%     vbasis = find(GurobivBasis==0);
%     vbasnum(k) = length(vbasis);
%     cbasis = find(GurobicBasis==-1);
...
%     ratio(k) = optface(k)/min(vbasnum(k),rank(k));
    ...
    [GurobiCostSub(k),GurobiOFSub,~,GurobiTimeSub(k),vbasis,cbasis] = gurLPSub_bar(A_mat,b,c_pt,l,u,eps,sense,sigma,tau);
     
    [GurobiCostFinal(k),GurobiOFFinal,GurobiTimeFinal(k)] = gurLPFinal(A_mat,b,c,l,u,sense,vbasis,cbasis);
...
%     cross_pt(k) = GurobiTimeBlurCross_pt(k) - GurobiTimeBlur_pt(k);
    ...
    timeall_pt(k) = GurobiTimeSub(k) + GurobiTimeFinal(k);
