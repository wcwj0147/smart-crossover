...
%% 初始化
%     cmd = sprintf('read(%s)', char(problems(k))); % Read the problem from file
%     [rcode, res] = mosekopt(cmd);
%     % mpsdata = mpsread(problems(k));
%     c = res.prob.c;
%     b_l = res.prob.blc;
%     b_u = res.prob.buc;
%     l = res.prob.blx;
...
%     end    
    ...
    [GurobiCostBlur,GurobiOFBlur,GurobiTimeBlur] = GurobiLPBlur(A_mat,b,c,l,u,eps,large_networkLP_benchmark(k));
