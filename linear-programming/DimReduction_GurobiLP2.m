function [realCost, realOF, time, count1] = DimReduction_GurobiLP2(A_mat,b,c,l,u,X)
...
for testtime = 1:N
    ...
    while flag
        ...
        [~,subOF,~,subTime,bas_1] = GurobiLPSub(A_mat_1,b_1,c_1,l_1,u_1,bas_1,T_1);
...
%         p1 = U'\c_B; p2 = L'\p1; p = P\p2;
        ...
        if max(sol(m+1:m+n).*sign(sol(m+1:m+n)))<1e-6
            flag = false;
        end    
    ...
    disp(' ');
    disp('        Artificial variables are eliminated.         ');
    disp(' ');
    bas = bas_1(1:m);
    [~, MyOF, t_f] = GurobiLPFinal(A_mat,b,c,l,u,bas);

    tdimReduction = t_1 + t_2 + t_3
    time(testtime) = tdimReduction + t_sort + t_f;
    realCost(testtime) = c'*MyOF{1};
    realOF{int16(testtime)} = MyOF;
