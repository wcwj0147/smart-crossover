...
CplexCostAll = zeros(length(problem),1);
CplexTimeAll = zeros(length(problem),1);
CplexCostBlurCross = zeros(length(problem),1);
CplexTimeBlurCross = zeros(length(problem),1);
CplexCostBlur = zeros(length(problem),1);
CplexTimeBlur = zeros(length(problem),1);
cross = zeros(length(problem),1);
CplexCostBlurCross_pt = zeros(length(problem),1);
CplexTimeBlurCross_pt = zeros(length(problem),1);
CplexCostBlur_pt = zeros(length(problem),1);
CplexTimeBlur_pt = zeros(length(problem),1);
CplexOFBlurCross = cell(length(problem),1);
CplexOFBlur = cell(length(problem),1);
CplexPi = cell(length(problem),1);
...
CplexCostFinal = zeros(length(problem),1);
CplexTimeFinal = zeros(length(problem),1);
...
%     continue;
    ...
    [CplexCostBlurCross(k),CplexOFBlurCross(k),CplexTimeBlurCross(k),CplexvBasis,CplexcBasis] = CplexLPBlurCross(A_mat,b,c,l,u,eps);
    
    [CplexCostBlur(k),CplexOFBlur(k),CplexTimeBlur(k)] = CplexLPBlur(A_mat,b,c,l,u,eps);
    
    vbasis = find(CplexvBasis==1);
    vbasnum(k) = length(vbasis);
    cbasis = find(CplexcBasis==0);
    cbasnum(k) = length(cbasis);
    
%     pp = CplexPi(k); pi = pp{1};
%     xx = CplexOFBlur(k); x = xx{1};
%     s = c - A_mat'*pi;
%     sigma = find(x>=s);
%     
%     optfacesize = length(sigma)
%     rank_A = min(vbasnum(k),rank(k))
%     Ratio = optfacesize/rank_A
%     optface(k) = length(sigma);
    
%     ratio(k) = optface(k)/min(vbasnum(k),rank(k));
    
%     if CplexTimeBlurCross(k) > 2*CplexTimeBlur(k)
%         long_cross(k) = 1;
%     end
    ...
    [CplexCostBlurCross_pt(k),CplexOFBlurCross_pt,CplexTimeBlurCross_pt(k),vbas,cbas] = CplexLPBlurCross(A_mat,b,c_pt,l,u,eps);
    
    [CplexCostFinal(k),CplexOFFinal,CplexTimeFinal(k)] = CplexLPFinal_read(A_mat,b,c,l,u);
    
    [CplexCostBlur_pt(k),CplexOFBlur_pt,CplexTimeBlur_pt(k)] = CplexLPBlur(A_mat,b,c_pt,l,u,eps);
    ...
    cross(k) = CplexTimeBlurCross(k) - CplexTimeBlur(k);
    cross_pt(k) = CplexTimeBlurCross_pt(k) - CplexTimeBlur_pt(k);
 
    error = CplexCostFinal(k) - CplexCostBlurCross(k);
    err(k) = error*sign(error);
    gapp = c.'*CplexOFBlurCross_pt{1}- CplexCostBlurCross(k);
